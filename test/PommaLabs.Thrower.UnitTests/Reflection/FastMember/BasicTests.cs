...
namespace PommaLabs.Thrower.UnitTests.Reflection.FastMember
{
    ...
    public class BasicTests
    {
        ...
        [Test]
        public void GetMembersOnStruct()
        {
            var access = TypeAccessor.Create<PropsOnStruct>();
            Assert.IsTrue(access.GetMembersSupported);
            var members = access.GetMembers();
            Assert.AreEqual(4, members.Count);
            Assert.AreEqual("A", members[0].Name);
        ...
        {
            var acc0 = TypeAccessor.Create(typeof(MixedAccess));
            var acc1 = TypeAccessor.Create(typeof(MixedAccess), false);
            var acc2 = TypeAccessor.Create(typeof(MixedAccess), true);
